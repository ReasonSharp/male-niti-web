...
### Legal pages (repo root, `hi-fi/legal-pages.jsx`)
Two more standalone entry points: **`impressum.html`** (renders `ImprintPage`) and **`privatnost.html`** (renders `PrivacyPage`). Same chain as the blog pages but with `hi-fi/legal-pages.jsx` swapped in for `hi-fi/blog-pages.jsx`. Both share the `LegalPage`/`LegalSection`/`Rows`/`P` layout primitives defined there. `ImprintPage` pulls the actual registration details (OIB, bank details, etc.) from `useApiItem('/imprint', DEFAULT_IMPRINT)` — any value still showing the bracketed `DEFAULT_IMPRINT` placeholder text renders wrapped in `<Fill>` (`.legal-fill` in `hi-fi/styles.css`) so a stale placeholder is visually obvious rather than shipping silently. There is no analytics, tracking, or cookie-consent mechanism anywhere on the site by design — `PrivacyPage`'s "Cookies and local storage" section reflects that; don't reintroduce one without updating that section to match.

### Content/API layer
`hi-fi/content.jsx` fetches live content from a backend (`window.MALE_NITI_CONFIG.apiBase`, default same-origin `/api`) via `useApiList` (lists) or `useApiItem` (single object — used by the blog post page and the imprint page), and **always falls back** to the bundled `DEFAULT_SERVICES` / `DEFAULT_PRICING` / `DEFAULT_BLOG` / `DEFAULT_WORK` / `DEFAULT_IMPRINT` constants on any network error, 404, or malformed response — the page must never show empty sections. `api-spec.yaml` is the OpenAPI contract for that backend (`/services`, `/pricing`, `/work`, `/blog`, `/blog/{slug}`, `/imprint`, `/contact`), plus `/feed.xml` (an RSS feed, routed at the site root rather than under `/api` — see the `servers` override on that path); there is no backend implementation in this repo. **When adding/changing a content field, update the `DEFAULT_*` fallback in `content.jsx` and the matching schema in `api-spec.yaml` together** — they're required to mirror each other (see the note at the top of `content.jsx`).
...
1. Upload only the shipping files — `Male Niti.html`, `blog.html`, `blog-post.html`, `impressum.html`, `privatnost.html`, `hi-fi/`, `api-spec.yaml` — to the self-hosting server, preserving the folder structure. (The root wireframe track — `index.html`, `design-canvas.jsx`, `app.jsx`, `variant-*.jsx`, `shared.jsx` — is design-exploration scaffolding and is *not* deployed.)
2. Serve `Male Niti.html` at `/` (rename or symlink to `index.html` as needed).
3. Stand up the backend per `api-spec.yaml`, mounted at `/api` on the same origin (matches `hi-fi/content.jsx`'s default `apiBase`) — or, if the API is hosted elsewhere, set `window.MALE_NITI_CONFIG = { apiBase: 'https://your-api-host' }` before the app scripts load in the HTML. The one exception is `/feed.xml`, which must be routed at the site root, not under `/api` (every page links it as plain `feed.xml`). The backend itself is being built separately, outside this repo, from `api-spec.yaml` as the contract — so changes to that spec here are the source of truth the API side needs to track.
4. Point maleniti.com's DNS/webserver at the host and add HTTPS.

There is no analytics/tracking script and no cookie-consent mechanism anywhere in this project, by design — see `hi-fi/legal-pages.jsx`'s `PrivacyPage`. Don't add one without updating that page's "Cookies and local storage" section to match.

## Conventions to follow

- **No ES modules.** Every file attaches its exports to `window` via `Object.assign(window, {...})` at the bottom (see `shared.jsx`, `hi-fi/components.jsx`, `hi-fi/content.jsx`, `tweaks-panel.jsx`). New shared helpers must follow this pattern, and new files must be added to the `<script type="text/babel" src="...">` list in the relevant HTML entry (`index.html`, `Male Niti.html`, `blog.html`, `blog-post.html`, `impressum.html`, or `privatnost.html`) **in dependency order** — a file using a global from another file must load after it.
