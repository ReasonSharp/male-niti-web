...
Entry point: **`Male Niti.html`** (note the space in the filename). Loads `tweaks-panel.jsx` (shared with the root track) then `hi-fi/content.jsx` → `hi-fi/components.jsx` → `hi-fi/sections-top.jsx` → `hi-fi/sections-bottom.jsx` → `hi-fi/app.jsx`, styled by `hi-fi/styles.css`.
...
- `hi-fi/app.jsx` composes the page: `Header → Hero → Services → Process → Work → About → Pricing → Blog → Contact → Footer`. `Header`/`Hero`/`Services`/`Process` live in `sections-top.jsx`; the rest live in `sections-bottom.jsx`.
...
1. Upload only the shipping files — `Male Niti.html`, `hi-fi/`, `api-spec.yaml` — to the self-hosting server, preserving the folder structure. (The root wireframe track — `index.html`, `design-canvas.jsx`, `app.jsx`, `variant-*.jsx`, `shared.jsx` — is design-exploration scaffolding and is *not* deployed.)
